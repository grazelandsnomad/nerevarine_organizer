...
# APPIMAGE_VERSION goes into the artifact filename; it's derived from
# CMakeLists.txt just below (once REPO_ROOT is known) so a release bumps the
# version in exactly one place - same approach as build-release.sh.
...
# Single-source the version from CMakeLists.txt (same grep as build-release.sh)
# so a release only bumps it in one place; it goes into the artifact filename.
APPIMAGE_VERSION=$(grep -m1 'project(NerevarineOrganizer VERSION' "$REPO_ROOT/CMakeLists.txt" \
                   | grep -oP '\d+\.\d+(?:\.\d+)?')
[[ -n "$APPIMAGE_VERSION" ]] || { echo "ERROR: could not read version from CMakeLists.txt" >&2; exit 1; }
