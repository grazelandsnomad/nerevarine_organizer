...
struct Settings {
    ...
    // Conflict arrows / "overwritten by X" captions / partner row wash in the
    // mod list. Default true; off leaves the list plain for users who know
    // their load order and want the space back.
    ...
    // By default the dropdown shows only the pinned() games (OpenMW, the
    // Bethesda deploy targets Oblivion / FNV / Fallout 4 / Starfield, and the
    // Enderal pair). This unhides the rest of the classified game list
    // (Skyrim, Fallout 3, the Witcher / Gothic / Dark Souls families, etc.).
    // Default false; a toolbar checkbox sets it.
