...
public:
    ...
    // User-initiated abort of a verify or extract in flight. Kills the live
    // extractor process if there is one; otherwise just flags the token so the
    // next controller step for it (MD5 continuation, extractArchive entry, or
    // a process-finished handler) bails out. Exactly one extractionCancelled
    // is emitted per consumed flag - never extractionSucceeded/Failed - even
    // when the extractor happened to finish cleanly before the kill landed
    // (the flag check runs first in every finished handler, so cancel wins
    // the race deterministically). Unknown/idle tokens are a harmless no-op.
    void cancelInstall(const QUuid &installToken);
...
signals:
    ...
    // cancelInstall() took effect. extractDir is empty when the cancel landed
    // before extraction started (verify phase / between verified and extract);
    // the slot must only clean it up when non-empty.
    void extractionCancelled(const QString &archivePath,
                             const QString &extractDir,
                             const QUuid &installToken);

private:
    // Consume a pending cancel for this token. One cancel -> one
    // extractionCancelled, so every check site uses this take-semantics call.
    bool takeCancelled(const QUuid &installToken);

    // Live extractor per token, so cancelInstall can kill it. QPointer because
    // the QProcess deleteLater()s itself from its finished handler.
    QHash<QUuid, QPointer<QProcess>> m_running;
