...
// "<compatdataRoot>/<appId>/pfx/user.reg" - the prefix's per-user registry,
// which is where a DLL override has to be recorded for a mod's wrapper DLL to
// be loaded at all (see dll_overrides).  Empty if either input is empty.
QString userRegPath(const QString &compatdataRoot, const QString &appId);
