...
// Same, plus extra environment variables layered on top of childEnvironment().
// For launching a Windows exe through Proton, which needs
// STEAM_COMPAT_DATA_PATH / STEAM_COMPAT_CLIENT_INSTALL_PATH to find the
// prefix - and still needs the AppImage scrub underneath, since Proton spawns
// its own Qt-free toolchain but inherits whatever we hand it.
bool startDetachedWithEnv(const QString &program, const QStringList &args,
                          const QList<QPair<QString, QString>> &extraEnv,
