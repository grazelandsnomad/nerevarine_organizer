...
public:
    ...
    // -- Nexus Mods -----------------------------------------------------
    // The <game> in https://www.nexusmods.com/<game>/mods/<id> and in
    // nxm://<game>/..., which is why nearly every id above is spelled as
    // its Nexus domain - the default here is id().
    //
    // Override only where a profile is not its own Nexus game. Skyrim AE
    // is the case: Nexus files AE mods under skyrimspecialedition, so an
    // AE profile must still build SE URLs or the MO2 import writes 404s.
    virtual QString nexusDomain() const { return id(); }
    ...
    // The single folder the engine loads content from, relative to the folder
    // holding the game .exe: "Data" for Bethesda, "." for engines that read
    // straight out of the exe's own folder (the Souls games). Non-empty is
    // also what makes the Deploy actions appear for a game.
    //
    ...
    // Name of the *Prefs.ini the "Tune INI" dialog edits (resolution,
    // display mode, VSync, shadow preset). It sits in the same
    // Documents/My Games/<myGamesName> dir as the engine ini. Empty = no
    // tuner, and the toolbar action hides.
    //
    // Was an `id == "skyrimspecialedition"` compare in the toolbar, so a
