...
// The app version is injected by CMake from project(... VERSION) in
// CMakeLists.txt, so there's a single place to bump it for a release. This
// fallback only kicks in for a build that compiled main.cpp without that
// define; the obviously-fake value makes such a build easy to spot.
#ifndef NR_APP_VERSION
#define NR_APP_VERSION "0.0-dev"
#endif
...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationVersion(QStringLiteral(NR_APP_VERSION));
