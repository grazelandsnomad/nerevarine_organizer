...
// Enderal's ids must equal the Nexus game domains - that's what routes nxm://
// downloads and the Wabbajack import table with no extra mapping - and the SE
// edition mirrors Skyrim SE's deploy classification while the LE-engine
// original stays modlists-only (no Deploy actions).
static void testEnderalClassification()
{
    std::cout << "\n[Enderal: SE fully classified, original modlists-only]\n";
    const GameAdapter *se = GameAdapterRegistry::find("enderalspecialedition");
    check("enderalspecialedition adapter exists (id == Nexus domain)", se != nullptr);
    if (se) {
        check("SE uses the '*'-prefixed Plugins.txt style",
              se->loadOrderStyle() == LoadOrderStyle::AsteriskPluginsTxt);
        check("SE declares Data/ so the deploy actions are visible",
              se->dataSubdir() == QStringLiteral("Data"), se->dataSubdir());
        check("SE has its own AppData/Local dir (not Skyrim SE's)",
              se->localAppDataName() == QStringLiteral("Enderal Special Edition"),
              se->localAppDataName());
        check("SE has its own My Games dir (not Skyrim SE's)",
              se->myGamesName() == QStringLiteral("Enderal Special Edition"),
              se->myGamesName());
        check("SKSE64 loader listed",
              se->scriptExtenderLoaders().contains(QStringLiteral("skse64_loader.exe")),
              se->scriptExtenderLoaders().join(", "));
        check("SE engine needs no archive ini (like Skyrim SE)",
              se->archiveConfig().style == GameAdapter::ArchiveConfig::Style::None);
        check("pinned so a profile can be created from the dropdown",
              se->pinned());
        check("stays out of the first-run chooser", !se->builtin());
    }
    const GameAdapter *le = GameAdapterRegistry::find("enderal");
    check("enderal adapter exists (id == Nexus domain)", le != nullptr);
    if (le) {
        check("original stays unclassified for deploy (LE engine)",
              le->loadOrderStyle() == LoadOrderStyle::Unknown);
        check("original hides the Deploy actions (no dataSubdir)",
              le->dataSubdir().isEmpty(), le->dataSubdir());
        check("pinned so a profile can be created from the dropdown",
              le->pinned());
    }
}
// Skyrim ships as three separately-modded things and used to be two adapters,
// one of them just called "Skyrim". These pin what makes each distinct: SE and
// AE share an engine and a Nexus section but never a profile, while Oldrim
// shares nothing with either. The subtle one is AE's nexusDomain(): its id is
// NOT a Nexus game, so anything building a URL from the profile id alone (the
// MO2 importer) writes 404s unless it goes through the adapter.
static void testSkyrimEditionsAreSeparate()
{
    std::cout << "\n[Skyrim: SE, AE and Oldrim are three separate games]\n";
    const GameAdapter *se = GameAdapterRegistry::find("skyrimspecialedition");
    const GameAdapter *ae = GameAdapterRegistry::find("skyrimanniversaryedition");
    const GameAdapter *le = GameAdapterRegistry::find("skyrim");
    check("all three adapters exist", se && ae && le);
    if (!se || !ae || !le) return;

    check("three distinct profile ids, so three mods dirs and modlists",
          se->id() != ae->id() && ae->id() != le->id() && se->id() != le->id());
    check("three distinct names, none of them a bare \"Skyrim\"",
          se->displayName() != ae->displayName()
              && ae->displayName() != le->displayName()
              && le->displayName() != QStringLiteral("Skyrim"),
          le->displayName());
    check("all three reachable without \"Show all games\"",
          se->pinned() && ae->pinned() && le->pinned());

    // AE is SE patched to 1.6.x: same engine, same exe, same everything that
    // faces outward. Inherited rather than restated, so this guards the
    // inheritance as much as the values.
    check("AE runs SE's exe from SE's Steam app",
          ae->steamAppId() == se->steamAppId()
              && ae->steamLayout().exe == se->steamLayout().exe,
          ae->steamAppId() + "/" + ae->steamLayout().exe);
    check("AE deploys like SE ('*'-prefixed Plugins.txt into Data/)",
          ae->loadOrderStyle() == LoadOrderStyle::AsteriskPluginsTxt
              && ae->dataSubdir() == QStringLiteral("Data"));
    check("AE shares SE's prefix dirs (one install, one set of configs)",
          ae->localAppDataName() == se->localAppDataName()
              && ae->myGamesName() == se->myGamesName(),
          ae->myGamesName());
    check("AE is the same LOOT game as SE",
          ae->lootSlug() == se->lootSlug(), ae->lootSlug());
    check("AE lists SKSE64, same as SE",
          ae->scriptExtenderLoaders() == se->scriptExtenderLoaders(),
          ae->scriptExtenderLoaders().join(", "));

    // The one thing AE must NOT take from its id.
    check("AE asks Nexus for skyrimspecialedition, not its own id",
          ae->nexusDomain() == QStringLiteral("skyrimspecialedition")
              && ae->nexusDomain() != ae->id(),
          ae->nexusDomain());
    check("SE's Nexus domain is still its id", se->nexusDomain() == se->id());

    // Oldrim: a different engine, and nothing shared with SE/AE.
    check("Oldrim is its own Steam app running TESV.exe",
          le->steamAppId() == QStringLiteral("72850")
              && le->steamLayout().exe == QStringLiteral("TESV.exe"),
          le->steamAppId() + "/" + le->steamLayout().exe);
    check("Oldrim is its own Nexus section and its own LOOT game",
          le->nexusDomain() == QStringLiteral("skyrim")
              && le->lootSlug() == QStringLiteral("Skyrim"));
    check("Oldrim lists 32-bit SKSE, not SE/AE's skse64",
          le->scriptExtenderLoaders() == QStringList{QStringLiteral("skse_loader.exe")},
          le->scriptExtenderLoaders().join(", "));
    check("Oldrim has its own config dirs, not SE's",
          le->myGamesName() == QStringLiteral("Skyrim")
              && le->myGamesName() != se->myGamesName());
    // loadorder.txt + plugins.txt is a third scheme with no writer here.
    // Claiming either existing style would deploy the mods and load them in
    // the wrong order, so Deploy stays hidden - as for Enderal's LE original.
    check("Oldrim stays unclassified for deploy (no writer for its scheme)",
          le->loadOrderStyle() == LoadOrderStyle::Unknown);
    check("Oldrim hides the Deploy actions (no dataSubdir)",
          le->dataSubdir().isEmpty(), le->dataSubdir());

    // prefsIniName drives the "Tune INI" toolbar action, which was gated on
    // an `id == "skyrimspecialedition"` compare - AE would have lost it.
    check("SE and AE both offer the INI tuner",
          se->prefsIniName() == QStringLiteral("SkyrimPrefs.ini")
              && ae->prefsIniName() == se->prefsIniName());
    check("Oldrim does not (tuner keys verified on SE only)",
          le->prefsIniName().isEmpty());
    check("a game with no tuner reports no prefs ini",
          GameAdapterRegistry::find("morrowind")->prefsIniName().isEmpty());
}

// Every id doubles as a Nexus domain unless the adapter says otherwise, and
// call sites that only have a profile id rely on that. Skyrim AE is the sole
// exception today; a second one added without the override would silently
// build dead URLs, so assert the rule holds for everyone else.
static void testNexusDomainDefaultsToId()
{
    std::cout << "\n[nexusDomain: id by default, overridden only for Skyrim AE]\n";
    QStringList overridden, blank;
    for (const GameAdapter *a : GameAdapterRegistry::all()) {
        if (a->nexusDomain().isEmpty()) blank << a->id();
        else if (a->nexusDomain() != a->id()) overridden << a->id();
    }
    check("no adapter reports an empty Nexus domain", blank.isEmpty(),
          blank.join(", "));
    check("exactly one adapter overrides its Nexus domain",
          overridden == QStringList{QStringLiteral("skyrimanniversaryedition")},
          overridden.join(", "));
}

// Dark Souls II: an engine with no plugins at all. It deploys by overlaying
// the folder that holds the .exe, which is dataSubdir "." - so the interesting
// property is that declaring it turns the Deploy actions on WITHOUT dragging in
// any of the Plugins.txt / archive-ini machinery.
static void testDarkSouls2Classification()
{
    std::cout << "\n[Dark Souls II: game-folder overlay, no plugin machinery]\n";
    for (const char *id : {"darksouls2sotfs", "darksouls2"}) {
        const GameAdapter *a = GameAdapterRegistry::find(QString::fromLatin1(id));
        check("adapter exists", a != nullptr, QString::fromLatin1(id));
        if (!a) continue;
        check("deploys into the exe's own folder (dataSubdir \".\")",
              a->dataSubdir() == QStringLiteral("."), a->dataSubdir());
        check("no plugin list to write", a->loadOrderStyle() == LoadOrderStyle::Unknown);
        check("no archive ini to configure",
              a->archiveConfig().style == GameAdapter::ArchiveConfig::Style::None);
        check("LOOT does not know this game", a->lootSlug().isEmpty(), a->lootSlug());
        check("no script extender", a->scriptExtenderLoaders().isEmpty());
        check("no separate launcher exe", !a->hasLauncher());
        check("stays out of the first-run chooser", !a->builtin());
        check("exe lives in the Game/ subfolder",
              a->steamLayout().exe == QStringLiteral("Game/DarkSoulsII.exe"),
              a->steamLayout().exe);
    }
    const GameAdapter *sotfs = GameAdapterRegistry::find("darksouls2sotfs");
    const GameAdapter *og    = GameAdapterRegistry::find("darksouls2");
    if (sotfs && og) {
        check("Scholar is pinned (the edition mods target)", sotfs->pinned());
        check("the original is not pinned (same support, one menu entry)",
              !og->pinned());
        check("separate Steam apps, so separate prefixes and profiles",
              sotfs->steamAppId() == QStringLiteral("335300")
                  && og->steamAppId() == QStringLiteral("236430"),
              sotfs->steamAppId() + "/" + og->steamAppId());
    }
}
} // namespace adapters_section
...
static void run_game_adapters()
{
    ...
    adapters_section::testSkyrimEditionsAreSeparate();
    adapters_section::testNexusDomainDefaultsToId();
    adapters_section::testEnderalClassification();
...
// -- dll_overrides ------------------------------------------------------------
// Splicing entries into a live Proton prefix's user.reg. The file holds the
// whole prefix's configuration, so the bar is: touch the one section, leave
// every other byte alone, and never take away an override the user set.

namespace dllov_section {

// Shaped after a real Proton prefix: a per-exe AppDefaults override section
// (which must never be confused for the global one), the global section Wine
// keeps alphabetically sorted, and an unrelated section after it.
static QString sampleReg()
{
    return QStringLiteral(
        "WINE REGISTRY Version 2\n"
        ";; All keys relative to \\\\User\\\\S-1-5-21-0\n"
        "\n"
        "[Software\\\\Wine\\\\AppDefaults\\\\ffxvi.exe\\\\DllOverrides] 1785137682\n"
        "#time=1dd1d9a637cf812\n"
        "\"atiadlxx\"=\"builtin\"\n"
        "\n"
        ...
        "\n"
        "[Software\\\\Wine\\\\Drivers] 1785137682\n"
        "\"Graphics\"=\"x11\"\n");
}

static void testWrapperDetection()
{
    std::cout << "\n[dll_overrides: which DLLs even need an override]\n";
    check("dxgi is a wrapper (DS2LightingEngine, ReShade)",
          dll_overrides::isWrapperDll("dxgi.dll"));
    check("dinput8 is a wrapper (Souls Mod Engine)",
          dll_overrides::isWrapperDll("dinput8.dll"));
    check("extension and case are irrelevant",
          dll_overrides::isWrapperDll("D3D11"));
    check("a mod's own payload DLL is not (loaded by path, needs nothing)",
          !dll_overrides::isWrapperDll("nvngx_dlss.dll"));

    const QStringList picked = dll_overrides::wrapperDllsIn(
        {"dxgi.dll", "DINPUT8.DLL", "nvngx_dlss.dll", "shader/inner.dll",
         "readme.txt", "dxgi.dll"});
    check("root-level wrappers only, lowercased, deduped, sorted",
          picked == QStringList({"dinput8", "dxgi"}), picked.join(", "));
    check("a DLL in a subfolder is not on the loader's search path",
          !picked.contains("inner"));
}

static void testAddKeepsTheFileIntact()
{
    std::cout << "\n[dll_overrides: adding an entry]\n";
    QStringList added, skipped;
    const QString out = dll_overrides::addOverrides(sampleReg(), {"dxgi"},
                                                    &added, &skipped);
    check("reports what it added", added == QStringList({"dxgi"}), added.join(", "));
    check("nothing skipped", skipped.isEmpty(), skipped.join(", "));
    check("entry written with native first so the game still starts without it",
          out.contains("\"dxgi\"=\"native,builtin\""));

    // Wine writes the section sorted; keep it that way or the next Wine save
    // reshuffles our line and the diff looks like something else moved.
    const int atl = out.indexOf("\"atl100\"=");
    const int dxgi = out.indexOf("\"dxgi\"=");
    const int ucrt = out.indexOf("\"ucrtbase\"=");
    check("inserted in alphabetical order", atl < dxgi && dxgi < ucrt,
          QStringLiteral("atl100=%1 dxgi=%2 ucrtbase=%3").arg(atl).arg(dxgi).arg(ucrt));

    check("the header line survives untouched",
          out.contains("[Software\\\\Wine\\\\DllOverrides] 1785137682\n"
                       "#time=1dd1d9a637ce01c\n"));
    check("the per-exe AppDefaults section is left alone",
          out.contains("[Software\\\\Wine\\\\AppDefaults\\\\ffxvi.exe\\\\DllOverrides]"));
    check("unrelated sections after it survive", out.contains("\"Graphics\"=\"x11\""));
    check("no entry landed above the #time comment",
          out.indexOf("#time=1dd1d9a637ce01c") < dxgi);
}

static void testExistingValueIsNeverTouched()
{
    std::cout << "\n[dll_overrides: an override the user set themselves]\n";
    // The user runs their own ReShade via d3d11, or has disabled a DLL on
    // purpose. Overwriting it would break their setup; recording it as "added"
    // would be worse - undeploy would then delete their setting.
    QStringList added, skipped;
    const QString out = dll_overrides::addOverrides(sampleReg(), {"atiadlxx"},
                                                    &added, &skipped);
    check("left as-is, reported as skipped", added.isEmpty() && skipped == QStringList({"atiadlxx"}),
          QStringLiteral("added=%1 skipped=%2").arg(added.join(","), skipped.join(",")));
    check("their value is preserved verbatim", out.contains("\"atiadlxx\"=\"disabled\""));
    check("file is byte-identical when there is nothing to add", out == sampleReg());
}

static void testRemoveIsExactAndScoped()
{
    std::cout << "\n[dll_overrides: taking an entry back out]\n";
    QStringList added;
    const QString withDxgi = dll_overrides::addOverrides(sampleReg(), {"dxgi"}, &added);
    QStringList removed;
    const QString back = dll_overrides::removeOverrides(withDxgi, added, &removed);
    check("reports what it removed", removed == QStringList({"dxgi"}), removed.join(", "));
    check("add then remove restores the file exactly", back == sampleReg());

    // Scoping: the same name exists in the per-exe section, which is not ours.
    QStringList removed2;
    const QString scoped = dll_overrides::removeOverrides(sampleReg(), {"atiadlxx"}, &removed2);
    check("removed from the global section", !scoped.contains("\"atiadlxx\"=\"disabled\""));
    check("the per-exe section's same-named entry stays",
          scoped.contains("\"atiadlxx\"=\"builtin\""));

    QStringList removed3;
    const QString noop = dll_overrides::removeOverrides(sampleReg(), {"dxgi"}, &removed3);
    check("removing something that isn't there is a no-op",
          removed3.isEmpty() && noop == sampleReg());
}

static void testCreatesSectionWhenPrefixHasNone()
{
    std::cout << "\n[dll_overrides: a prefix with no overrides yet]\n";
    const QString bare = QStringLiteral(
        ...
        "\"Graphics\"=\"x11\"\n");
    QStringList added;
    const QString out = dll_overrides::addOverrides(bare, {"dxgi.dll"}, &added);
    check("section created", out.contains("[Software\\\\Wine\\\\DllOverrides]"));
    check("entry present", out.contains("\"dxgi\"=\"native,builtin\""));
    check("reported as added", added == QStringList({"dxgi"}), added.join(", "));
    check("the existing section is untouched", out.contains("\"Graphics\"=\"x11\""));
    check("file still ends in a newline", out.endsWith(QLatin1Char('\n')));

    // Re-reading what we wrote must find it, or a second deploy would add a
    // duplicate key and undeploy would only ever remove one of them.
    QStringList added2, skipped2;
    const QString twice = dll_overrides::addOverrides(out, {"dxgi"}, &added2, &skipped2);
    check("adding twice is idempotent",
          added2.isEmpty() && skipped2 == QStringList({"dxgi"}) && twice == out);
}

} // namespace dllov_section

static void run_dll_overrides()
{
    std::cout << "=== dll_overrides (Proton prefix user.reg) tests ===\n";

    dllov_section::testWrapperDetection();
    dllov_section::testAddKeepsTheFileIntact();
    dllov_section::testExistingValueIsNeverTouched();
    dllov_section::testRemoveIsExactAndScoped();
    dllov_section::testCreatesSectionWhenPrefixHasNone();
}
