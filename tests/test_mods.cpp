...
static void run_install_layout()
{
    ...
    // Bethesda Data/ subfolders, same rule. The script-extender dirs are the
    // ones this bit in practice: Address Library ships as nothing but
    // SKSE/Plugins/versionlib-*.bin, so the entire archive is a single
    // top-level SKSE/. Diving in put the files in Data/Plugins/ rather than
    // Data/SKSE/Plugins/, and the game reported a missing address library.
    expectDive("does not dive into bare SKSE/",      {"SKSE"},      {}, QString());
    expectDive("does not dive into bare skse/",      {"skse"},      {}, QString());
    expectDive("does not dive into bare F4SE/",      {"F4SE"},      {}, QString());
    expectDive("does not dive into bare OBSE/",      {"OBSE"},      {}, QString());
    expectDive("does not dive into bare NVSE/",      {"NVSE"},      {}, QString());
    expectDive("does not dive into bare SFSE/",      {"SFSE"},      {}, QString());
    expectDive("does not dive into bare Interface/", {"Interface"}, {}, QString());
    expectDive("does not dive into bare Strings/",   {"Strings"},   {}, QString());
    expectDive("does not dive into bare Source/",    {"Source"},    {}, QString());
    // A wrapper that merely CONTAINS one still dives - the rule is about the
    // wrapper's own name, not what is inside it.
    expectDive("still dives into a wrapper holding SKSE/",
...
// === variant_picker ===
//
// Modelled directly on Main Menu Redone: Data/mainmenuwallpapers/<Option>/DATA
// with the option trees spelling Data as upper-case "DATA" while the mod's own
// root spells it "Data" - the exact case hazard resolveDest exists for.
...
static void testVariantChooserDetected()
{
    std::cout << "\n[variant_picker: the Main Menu Redone shape is found]\n";
    QTemporaryDir d;
    const QString root = d.path();
    write(root + "/Data/textures/interface/objects/vim.dds", "logo");
    write(root + "/Data/mainmenuwallpapers/Blackreach/DATA/textures/interface/objects/wall.dds", "black");
    write(root + "/Data/mainmenuwallpapers/AuroraBorealis/DATA/textures/interface/objects/wall.dds", "aurora");
    write(root + "/Data/mainmenuwallpapers/AuroraBorealis/DATA/music/special/mus_maintheme.xwm", "tune");

    const auto ch = variant_picker::find(root);
    check("chooser found", !ch.dir.isEmpty(), ch.dir);
    check("chooser is the wallpapers folder",
          QFileInfo(ch.dir).fileName() == "mainmenuwallpapers", ch.dir);
    check("destination is the folder holding the chooser (the mod's Data/)",
          QFileInfo(ch.destDir).fileName() == "Data", ch.destDir);
    check("both options listed, sorted",
          ch.options == QStringList({"AuroraBorealis", "Blackreach"}),
          ch.options.join(", "));
}

static void testVariantApplyMergesCaseInsensitively()
{
    std::cout << "\n[variant_picker: applying merges DATA/ into Data/, no fork]\n";
    QTemporaryDir d;
    const QString root = d.path();
    write(root + "/Data/textures/interface/objects/vim.dds", "logo");
    write(root + "/Data/mainmenuwallpapers/Blackreach/DATA/textures/interface/objects/wall.dds", "black");
    ...
    const auto ch  = variant_picker::find(root);
    const auto res = variant_picker::apply(ch, "AuroraBorealis");
    check("one file copied, no errors",
          res.copied == 1 && res.errors.isEmpty(),
          QString::number(res.copied) + " / " + res.errors.join("; "));
    QFile out(root + "/Data/textures/interface/objects/wall.dds");
    check("landed inside the EXISTING Data/textures tree",
          out.open(QIODevice::ReadOnly) && out.readAll() == "aurora");
    // The whole point of routing through resolveDest: the option spelling
    // "DATA" upper-case must not fork a second tree next to "Data".
    check("no case-forked DATA/ created beside Data/",
          !QDir(root + "/DATA").exists());
    check("existing unrelated file untouched",
          [&]{ QFile f(root + "/Data/textures/interface/objects/vim.dds");
               return f.open(QIODevice::ReadOnly) && f.readAll() == "logo"; }());

    // Re-picking replaces: last-writer-wins, the choice stays revisable.
    const auto res2 = variant_picker::apply(ch, "Blackreach");
    QFile out2(root + "/Data/textures/interface/objects/wall.dds");
    check("re-picking another option overwrites",
          res2.copied == 1 && out2.open(QIODevice::ReadOnly)
              && out2.readAll() == "black");
}

static void testVariantChooserStrictness()
{
    std::cout << "\n[variant_picker: strictness - normal mods never match]\n";
    // A texture pack whose subfolders merely organise content: no data/
    // children, so not a chooser.
    {
        QTemporaryDir d;
        write(d.path() + "/Data/textures/armor/iron/cuirass.dds", "x");
        write(d.path() + "/Data/textures/armor/steel/cuirass.dds", "y");
        check("plain texture pack has no chooser",
              variant_picker::find(d.path()).dir.isEmpty());
    }
    // One single option is not a choice.
    {
        QTemporaryDir d;
        write(d.path() + "/opts/OnlyOne/Data/textures/a.dds", "x");
        check("a single option is not a chooser",
              variant_picker::find(d.path()).dir.isEmpty());
    }
    // One subfolder without data/ disqualifies the whole candidate.
    {
        QTemporaryDir d;
        write(d.path() + "/opts/A/Data/textures/a.dds", "x");
        write(d.path() + "/opts/B/Data/textures/b.dds", "y");
        write(d.path() + "/opts/docs/readme.txt", "hi");
        check("a stray non-option subfolder disqualifies",
              variant_picker::find(d.path()).dir.isEmpty());
    }
}

static void run_variant_picker()
{
    std::cout << "\n=== variant_picker ===\n";
    testVariantChooserDetected();
    testVariantApplyMergesCaseInsensitively();
