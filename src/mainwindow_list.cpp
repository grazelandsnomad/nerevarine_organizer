...
void MainWindow::updateConflictNoticesButton()
{
    if (!m_conflictNoticesBtn) return;
    const bool on = Settings::conflictNoticesVisible();
    // Label names what the click does, matching the theme button next to it.
    m_conflictNoticesBtn->setText(on ? T("toolbar_conflicts_hide")
                                     : T("toolbar_conflicts_show"));
    m_conflictNoticesBtn->setToolTip(T("toolbar_conflicts_tip"));
}

void MainWindow::onToggleConflictNotices()
{
    const bool on = !Settings::conflictNoticesVisible();
    Settings::setConflictNoticesVisible(on);
    if (m_delegate) m_delegate->setConflictNoticesVisible(on);
    updateConflictNoticesButton();
...
void MainWindow::onToggleUntranslatedNotices()
{
    ...
    // Unlike the overwrite notices, the data behind this one is not lying
    // around already: switching on has to go and read the plugins, and
    // switching off has to drop what the last scan decided so a later edit
    // can't repaint a stale verdict.
    if (on) {
        // No status-bar line: runTranslationScan puts the progress panel up
        // over the list, which is where the user is actually looking. This one
        // ends on a result the user has to dismiss - on a small list the scan
        // finishes in well under a second, and a panel that flashes past leaves
        // no way to tell "nothing needs translating" from "nothing happened".
        m_translationResultWanted = true;
...
// "Conflicts" submenu: one entry per mod this one shares files with, worded by
// what the click does rather than by the mechanism. The list shows who wins;
// this is how it gets changed without hunting for a row that may be hundreds
// of positions away.
void MainWindow::buildConflictMenu(QMenu *menu, QListWidgetItem *item)
{
    if (!menu || !item) return;
    const QString path = item->data(ModRole::ModPath).toString();
    const conflict_direction::Directions d = m_conflicts.value(path);
    const QList<plugin_records::RecordClash> clashes = m_recordClashes.value(path);
    if (d.isEmpty() && clashes.isEmpty()) return;

    auto *sub = menu->addMenu(T("ctx_conflicts"));

    // Currently losing to these: moving below the counterpart flips it.
    for (const auto &c : d.overwrittenBy) {
        const QString target = c.path;
        sub->addAction(T("ctx_conflict_take_over").arg(c.name), this,
                       [this, item, target] {
            moveModAdjacentTo(item, target, /*after=*/true);
        });
    }
    // Currently winning against these: moving above hands the files back.
    for (const auto &c : d.overwrites) {
        const QString target = c.path;
        sub->addAction(T("ctx_conflict_give_way").arg(c.name), this,
                       [this, item, target] {
            moveModAdjacentTo(item, target, /*after=*/false);
        });
    }

    // Record clashes get no reorder entry: moving a row does not settle them,
    // the plugin load order does. Disabling one of the two is what the user
    // actually wants when a translation duplicates its original.
    QList<QPair<QString, QString>> reveal;   // (name, path)
    for (const auto &c : d.overwrittenBy + d.overwrites) reveal << qMakePair(c.name, c.path);
    ...
    sub->addSeparator();
    for (const auto &c : reveal) {
        const QString target = c.second;
        sub->addAction(T("ctx_conflict_reveal").arg(c.first), this,
                       [this, target] {
            for (int i = 0; i < m_modList->count(); ++i) {
                auto *row = m_modList->item(i);
                if (row->data(ModRole::ModPath).toString() != target) continue;
                ...
                m_modList->setCurrentItem(row);
                m_modList->scrollToItem(row, QAbstractItemView::PositionAtCenter);
                break;
            }
        });
    }
    ...
    if (item) {
        ...
        if (isSep) {
            ...
            if (installStatus == 1) {
                ...
                // so no installer ever asked. Offer the choice here. The scan
                // is a handful of directory listings, cheap enough to run on
                // every menu open; the entry simply doesn't appear for the
                // overwhelming majority of mods that have no chooser.
                {
                    const auto chooser = variant_picker::find(
                        item->data(ModRole::ModPath).toString());
                    if (!chooser.dir.isEmpty()) {
                        menu.addAction(T("ctx_choose_variant"), this,
                                       [this, chooser]{
                            bool ok = false;
                            const QString picked = QInputDialog::getItem(
                                this, T("variant_dialog_title"),
                                T("variant_dialog_label")
                                    .arg(QFileInfo(chooser.dir).fileName()),
                                chooser.options, 0, /*editable=*/false, &ok);
                            if (!ok || picked.isEmpty()) return;
                            const auto res = variant_picker::apply(chooser, picked);
                            if (!res.errors.isEmpty()) {
                                ui::warn(this, T("variant_dialog_title"),
                                         res.errors.join('\n'));
                                return;
                            }
            ...
            if (installStatus == 2) {
                // Download phase (queued or transferring) is the queue's to
                // cancel; after the handoff the row belongs to the
                // InstallController (verify/extract), reached by its token.
                // Both cover the big-mod case where each phase runs minutes.
                if (m_downloadQueue->isQueued(item)) {
                    menu.addAction(T("ctx_cancel_download"), this, [this, item]{
                        m_downloadQueue->cancelQueued(item);
                    });
                } else if (!item->data(ModRole::InstallToken).toUuid().isNull()) {
                    menu.addAction(T("ctx_cancel_install"), this, [this, item]{
                        m_installCtl->cancelInstall(
                            item->data(ModRole::InstallToken).toUuid());
                    });
                }
            }
...
//
// Extracting every plugin of a mod is a full read of each one, and the first
// call also builds the vanilla table - a walk of Morrowind and its expansions,
// about 94 MB. Both used to run on the UI thread, so on a Project
// Cyrodiil-sized mod the app froze before the dialog even opened: the very
// first thing the translate workflow showed a user was a hang.
//
// Value types only, nothing Qt-widget shaped, so the whole struct crosses the
// thread boundary by copy. vanillaText() is safe to call here: it is a magic
// static, so a concurrent first call from anywhere serialises on construction
// and every later call reads an immutable table.
struct TranslateGather {
...
TranslateGather gatherTranslateStrings(const QString &modPath,
                                       std::shared_ptr<std::atomic<bool>> cancel)
{
    ...
    while (dit.hasNext()) {
        // Between plugins, not mid-extract: one plugin is the unit of work,
        // and a cancelled gather is thrown away wholesale anyway.
        if (cancel && cancel->load()) return g;
        dit.next();
        ...
        if (set.localized) { g.sawLocalized = true; continue; }
        const QString rel = QDir(modPath).relativeFilePath(dit.filePath());
        g.encodings.insert(rel, set.encoding);
        for (auto it = set.byKey.cbegin(); it != set.byKey.cend(); ++it) {
            ...
            g.strings.append({rel, it.key(), it.value(), false});
        }
        for (auto it = set.auxByKey.cbegin(); it != set.auxByKey.cend(); ++it) {
            noteVanilla(it.key(), it.value());
            g.strings.append({rel, it.key(), it.value(), true});
        }
    }

    for (auto it = vanillaSaysIt.cbegin(); it != vanillaSaysIt.cend(); ++it)
        if (it.value()) g.vanillaAnswers.insert(it.key());
    return g;
}
...
// wait for construction rather than redo it, which is never worse than today.
...
void MainWindow::onTranslateMod(QListWidgetItem *item)
{
    ...
    // The disk work - every plugin extracted, the vanilla table consulted -
    // runs on a worker behind a cancellable progress dialog, following
    // conflict_inspector's shape. minimumDuration keeps the dialog invisible
    // for the small mods that dominate; a big one shows a spinner instead of
    // freezing the whole window before it even opens.
    auto cancel = std::make_shared<std::atomic<bool>>(false);
    QProgressDialog progress(T("translate_scanning").arg(modName),
                             T("translate_scanning_cancel"), 0, 0, this);
    progress.setWindowTitle(T("translate_title").arg(modName));
    progress.setWindowModality(Qt::WindowModal);
    progress.setMinimumDuration(150);
    progress.setAutoClose(false);
    progress.setAutoReset(false);

    QFutureWatcher<TranslateGather> watcher;
    connect(&watcher, &QFutureWatcherBase::finished,
            &progress, &QProgressDialog::accept);
    connect(&progress, &QProgressDialog::canceled, &progress,
            [cancel] { cancel->store(true); });

    watcher.setFuture(QtConcurrent::run(gatherTranslateStrings, modPath, cancel));
    progress.exec();
    watcher.waitForFinished();   // the worker must be done before `g` is read
    if (cancel->load()) return;

    const TranslateGather g = watcher.result();
    const QList<TranslatableString>         &strings   = g.strings;
    const translation_mod::EncodingByPlugin &encodings = g.encodings;
    ...
    if (strings.isEmpty()) {
        ui::info(this, T("translate_title").arg(modName),
                 g.sawLocalized ? T("translate_localized").arg(modName)
    ...
    TranslateDialog dlg(modName, strings, language, &memory, rulesPath,
                        progressPath, g.vanillaAnswers, this);
