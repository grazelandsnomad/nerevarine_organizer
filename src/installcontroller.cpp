...
bool InstallController::takeCancelled(const QUuid &installToken)
{
    return m_cancelled.remove(installToken);
}

void InstallController::cancelInstall(const QUuid &installToken)
{
    if (installToken.isNull()) return;
    m_cancelled.insert(installToken);
    // Everything runs on the GUI thread, so this can't interleave with
    // extractArchive itself - only with the queued finished/continuation
    // handlers, all of which check the flag before anything else.
    if (QProcess *p = m_running.value(installToken))
        p->kill();
}
                                      ...
                                      qint64 expectedSize)
{
    ...
    async::guarded(this,
        ...
        [archivePath, installToken, expectedMd5Lower](
            ...
            // A cancel during the (minutes-long, for big archives) hash beats
            // both outcomes: don't proceed to extraction, don't show a
            // mismatch dialog for an install the user already gave up on.
            if (self->takeCancelled(installToken)) {
                emit self->extractionCancelled(archivePath, QString(),
                                               installToken);
                return;
            }
...
{
    // Cancel landed between verified() and this queued call: bail before any
    // filesystem work (in particular before the reuse-wrapper wipe below).
    if (takeCancelled(installToken)) {
        emit extractionCancelled(archivePath, QString(), installToken);
        return;
    }
    ...
    if (fmt == archive_magic::Format::Zip) {
        ...
        connect(proc, &QProcess::finished, this,
            ...
            m_running.remove(installToken);
            proc->deleteLater();
            // Cancelled while unrar ran: report it, and in particular do NOT
            // fall through to the 7z retry - the kill produces a nonzero exit
            // that would otherwise look like "unrar failed".
            if (takeCancelled(installToken)) {
                emit extractionCancelled(archivePath, extractDir, installToken);
                return;
            }
