...
// Every QSettings key string lives here. The rest of the codebase goes
// through these accessors, except migrations that read old key literals
// with no accessor left.
...
// Old keys, migrated out of QSettings on first launch.
...
// Forbidden mods: read-only, still waiting on migration to the portable file.
...
{
    // No default: the registry builds the per-game one (old `mods/dir` for
    // morrowind, `~/Games/<id>_mods` otherwise).
...
QString Settings::dataDir(const QString &gameId)
{
    return QSettings().value(gameKey(gameId, "data_dir")).toString();
}

void Settings::setDataDir(const QString &gameId, const QString &dir)
{
    QSettings().setValue(gameKey(gameId, "data_dir"), dir);
}

QString Settings::pluginsTxtPath(const QString &gameId)
{
    return QSettings().value(gameKey(gameId, "plugins_txt_path")).toString();
}

void Settings::setPluginsTxtPath(const QString &gameId, const QString &path)
{
    QSettings().setValue(gameKey(gameId, "plugins_txt_path"), path);
}
...
{
    // remove(prefix) drops the prefix plus every nested key.
...
bool Settings::conflictNoticesVisible()
{
    return QSettings().value(kUiConflictNotices, true).toBool();
}

void Settings::setConflictNoticesVisible(bool visible)
{
    QSettings().setValue(kUiConflictNotices, visible);
}

bool Settings::watchDownloads()
{
    return QSettings().value(kDownloadsWatch, true).toBool();
}
