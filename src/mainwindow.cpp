    ...
    : QMainWindow(parent)
{
    ...
    connect(m_installCtl, &InstallController::extractionCancelled,
            this, &MainWindow::onExtractionCancelled);
    ...
    connect(m_loadCtl, &LoadOrderController::translationsScanned,
            this, &MainWindow::onTranslationsScanned);
    connect(m_loadCtl, &LoadOrderController::translationScanProgress,
            this, [this](int percent) {
        if (!m_translationOverlay) return;
        // A scan buffered mid-flight re-fires from inside the controller, not
        // from runTranslationScan, so its panel would never be put up - the
        // second pass would run invisibly, which is the exact complaint the
        // panel exists to answer. Every scan opens with a 0, so treat that as
        // the start signal wherever it came from.
        if (!m_translationOverlay->isRunning())
            m_translationOverlay->begin(T("untranslated_scanning"));
        m_translationOverlay->setPercent(percent);
    });
    ...
    // Translation-coverage debounce. Much longer than the conflict scan's
    // 400ms: this one reads record bodies, so it should not chase every
    // keystroke of a drag-reorder. Only armed while the toggle is on.
    m_translationTimer = new QTimer(this);
    m_translationTimer->setSingleShot(true);
    m_translationTimer->setInterval(1500);
    connect(m_translationTimer, &QTimer::timeout,
            this, &MainWindow::runTranslationScan);
    // First scan for a user who left the toggle on last session.
    scheduleTranslationScan();
    // And tell a Bethesda profile straight away if its list has never reached
    ...
    // Pre-warm QMessageBox: standardPixmap/QIcon::fromTheme/the box itself page
    // in cold off FUSE, adding 200-500 ms to the first dialog. Deferred 100 ms
    // past first paint.
    //
    // AppImage only, which the comment always claimed and the code never
    // checked. A normal install or dev build reads all of this off the local
    // filesystem with nothing to warm, so it was paying for the warmup and, on
    // Wayland, for a stray dialog with it.
    if (!qEnvironmentVariableIsEmpty("APPIMAGE")) {
        QTimer::singleShot(100, this, [this]() {
            ...
            // grab(), never show(). WA_DontShowOnScreen is an X11-era promise
            // Wayland does not keep: show() commits a surface and the
            // compositor maps it regardless, which is how a textless Yes/No
            // dialog appeared at every startup. grab() polishes, lays out and
            // renders the box into a pixmap without ever creating a platform
            // window, so there is nothing a compositor could show.
...
void MainWindow::onCurrentModChanged(QListWidgetItem *current, QListWidgetItem * /*previous*/)
{
    ...
    if (!current || current->data(ModRole::ItemType).toString() != ItemType::Mod) {
        applyConflictHighlights(nullptr);
    ...
    applyConflictHighlights(current);
...
// Maps our per-profile ID to the Nexus game domain to build mod URLs with.
// Nearly always the same string - the ids are deliberately spelled as Nexus
// domains - but not always: a Skyrim AE profile has to ask Nexus for
// skyrimspecialedition, since AE has no section of its own.  Unknown profile
// ids fall back to the id, which is what the call sites used to do outright.
QString nexusDomainFor(const QString &profileId)
{
    if (const GameAdapter *a = GameAdapterRegistry::find(profileId))
        return a->nexusDomain();
    return profileId;
}
...
void MainWindow::switchToModlistProfile(int idx)
{
    ...
    // Same reasoning as switchToGame: a profile is a different mod set, so any
    // banner about the current one stops being true the moment we switch.
...
void MainWindow::scheduleConflictScan()
{
    ...
    // Every "the list changed" edge already routes here, so chaining the
    // translation scan on rather than repeating it at a dozen call sites keeps
...
void MainWindow::runConflictScan()
{
    ...
    //
    // rowOrderForPersist(), not the raw rows: which mod wins a shared file is
    // decided by the LOAD order, and a temporary Size/Date view sort must not
    // flip the arrows for rows it only moved on screen.
    QList<conflict_direction::Mod> mods;
    for (int idx : rowOrderForPersist()) {
        auto *it = m_modList->item(idx);
        if (it->data(ModRole::ItemType).toString() != ItemType::Mod) continue;
        ...
        mods.append({path, it->text()});
    }
    m_loadCtl->scanConflicts(mods);
}

void MainWindow::onConflictsScanned(
    const QHash<QString, conflict_direction::Directions> &res,
    const QHash<QString, QList<plugin_records::RecordClash>> &recordClashes)
{
    // "Name\tfile1\tfile2..." per counterpart - the packing the delegate
    // tooltip unpacks.
    auto packEntries = [](const QList<conflict_direction::Counterpart> &cps) {
        QStringList out;
        out.reserve(cps.size());
        for (const auto &c : cps) {
            QString entry = c.name;
            for (const QString &f : c.files) entry += '\t' + f;
            out << entry;
        }
        return out;
    };
    // "Name\t<shared>\tplugin.esm..." - same tab convention, count first.
    auto packClashes = [](const QList<plugin_records::RecordClash> &cs) {
        QStringList out;
        out.reserve(cs.size());
        for (const auto &c : cs) {
            QString entry = c.modName + '\t' + QString::number(c.shared);
            for (const QString &p : c.pluginNames) entry += '\t' + p;
            out << entry;
        }
        return out;
    };
    ...
    for (int i = 0; i < m_modList->count(); ++i) {
        ...
        if (it->data(ModRole::ItemType).toString() != ItemType::Mod) continue;
        const QString path = it->data(ModRole::ModPath).toString();
        const auto found   = res.constFind(path);
        const auto clashed = recordClashes.constFind(path);
        const bool anyFile   = (found   != res.constEnd());
        const bool anyRecord = (clashed != recordClashes.constEnd());

        it->setData(ModRole::HasConflict, anyFile || anyRecord);
        it->setData(ModRole::ConflictOverwrites,
                    anyFile ? packEntries(found->overwrites)    : QStringList());
        it->setData(ModRole::ConflictOverwrittenBy,
                    anyFile ? packEntries(found->overwrittenBy) : QStringList());
        it->setData(ModRole::ConflictSameRecords,
                    anyRecord ? packClashes(*clashed) : QStringList());
        if (anyFile || anyRecord) ++conflictCount;
    }

    // Selection highlights were computed against the previous scan.
    applyConflictHighlights(m_modList->currentItem());
...
void MainWindow::scheduleTranslationScan()
{
...
void MainWindow::runTranslationScan()
{
    ...
    // whom. Coverage doesn't actually depend on order today, but feeding the
    // two scans different worlds is how they drift apart.
    QList<conflict_direction::Mod> mods;
    for (int idx : rowOrderForPersist()) {
        auto *it = m_modList->item(idx);
        if (it->data(ModRole::ItemType).toString() != ItemType::Mod) continue;
    ...
    // Panel up before the worker starts, not after the first progress tick:
    // opening a plugin is the slow part, so waiting for a tick would leave the
    ...
    const QHash<QString, TranslationCoverage> &byModPath)
{
    ...
    for (int i = 0; i < m_modList->count(); ++i) {
        ...
        const bool none = (found->state == TranslationCoverage::State::NoTranslation);
        it->setData(ModRole::TranslationState, none ? 1 : 2);
        if (none) ++noTranslation;
        // "partner\ttranslatable\tidentical\tcommon", the plugins, a blank
        // separator field, then the samples - unpacked by the delegate tooltip.
        QStringList detail;
        detail << found->partnerName + '\t' + QString::number(found->translatable)
                      + '\t' + QString::number(found->identical)
                      + '\t' + QString::number(found->common);
        detail += found->pluginNames;
        detail << QString();
        detail += found->samples;
        it->setData(ModRole::TranslationDetail, detail);
        ++flagged;
    }
    ...
    if (!m_translationOverlay) return;
    if (!m_translationResultWanted) {
        // Background re-run after a list edit: the rows are already updated,
        // which is the whole answer. Popping a panel per checkbox click would
        // make the toggle unusable.
        m_translationOverlay->finish();
        return;
    }
    m_translationResultWanted = false;

    QString msg;
    if (flagged == 0) {
        msg = T("untranslated_result_none");
    } else if (noTranslation == flagged) {
        msg = T("untranslated_result_none_found").arg(flagged);
    } else if (noTranslation == 0) {
        msg = T("untranslated_result_partial_only").arg(flagged);
    } else {
...
// Paint the selected mod's conflict partners in place: the rows it overwrites
// and the rows that overwrite it, in the two directions' colours. Cleared for a
// separator or empty selection.
void MainWindow::applyConflictHighlights(QListWidgetItem *current)
{
    const QString selPath =
        (current && current->data(ModRole::ItemType).toString() == ItemType::Mod)
            ? current->data(ModRole::ModPath).toString() : QString();
    const conflict_direction::Directions d = m_conflicts.value(selPath);

    QSet<QString> loses, wins;
    for (const auto &c : d.overwrites)    loses.insert(c.path);
    for (const auto &c : d.overwrittenBy) wins.insert(c.path);

    for (int i = 0; i < m_modList->count(); ++i) {
        auto *it = m_modList->item(i);
        const QString path = it->data(ModRole::ModPath).toString();
        int v = 0;
        if (!selPath.isEmpty() && !path.isEmpty() && path != selPath) {
            if (loses.contains(path))     v = 1;   // selected shadows this row
            else if (wins.contains(path)) v = 2;   // this row shadows selected
        }
        it->setData(ModRole::ConflictHighlight, v);
    }
}

// Flipping a conflict is just a move: the lower mod's copy of a shared file is
// the one that loads, so putting `mod` directly under its counterpart makes it
// win, and directly above makes it lose. Moving it adjacent (rather than
// swapping the two rows) leaves the rest of the order untouched, which matters
// when the counterpart is 200 rows away in a curated list.
void MainWindow::moveModAdjacentTo(QListWidgetItem *mod, const QString &targetPath,
                                   bool after)
{
    if (!mod || targetPath.isEmpty()) return;

    int targetRow = -1;
    for (int i = 0; i < m_modList->count(); ++i) {
        if (m_modList->item(i)->data(ModRole::ModPath).toString() == targetPath) {
            targetRow = i;
            break;
        }
    }
    const int fromRow = m_modList->row(mod);
    if (targetRow < 0 || fromRow < 0 || targetRow == fromRow) return;

    m_undoStack->pushUndo();
    // Same as a drag-reorder: a deliberate move commits the displayed order as
    // the saved one, otherwise resetting a live Size/Date sort would throw the
    // move away with it. Clears the stamps only - rows do not move, so the
    // indices above stay valid.
    dropViewSortKeepingOrder();

    QListWidgetItem *taken = m_modList->takeItem(fromRow);
    if (fromRow < targetRow) --targetRow;          // rows above shifted up
    m_modList->insertItem(after ? targetRow + 1 : targetRow, taken);

    m_modList->clearSelection();
    taken->setSelected(true);
    m_modList->setCurrentItem(taken, QItemSelectionModel::NoUpdate);
    m_modList->scrollToItem(taken);
