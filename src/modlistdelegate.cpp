                            ...
                            const QModelIndex &index) const
{
    ...
    //
    // The untranslated wash below does NOT make a row dark: at alpha 105 over a
    // white base it lands around lightness 200 - light pink, not deep red - so
    // the caption on it still wants the deep tone. Checked by rendering it.
    ...
    // +2px slack on each half. Reserving exactly horizontalAdvance() means the
    // draw below elides at its own natural width: hinting rounds the painted
    // run a hair wider than the metric, so a caption that fits perfectly still
    // came out as "no translati..." with half the row empty beside it.
    constexpr int kCapSlack = 2;
    int capWinW  = capWin.isEmpty()  ? 0 : captionFm.horizontalAdvance(capWin)  + kCapSlack;
    int capLoseW = capLose.isEmpty() ? 0 : captionFm.horizontalAdvance(capLose) + kCapSlack;
    ...
    // Untranslated wash. Unlike the two above this one is STANDING, not
    // selection-relative: "nothing translates this mod" is true whatever row
    // happens to be selected, and it is the answer the user turned the toggle
    // on to get - a caption alone is easy to miss when scanning a long list.
    //
    // Red for no translation at all, amber for a partial one, matching the
    // caption. Yields to either selection-relative wash, which is about the row
    // the user is looking at right now; the toggle can simply be turned off if
                                ...
                                const QModelIndex &index)
{
        ...
        index.data(ModRole::ItemType).toString() == ItemType::Mod)
    {
        ...
        // Translation coverage has no icon slot of its own - it writes into the
        // caption - so it answers to the whole name zone instead, which is
        ...
        if (m_untranslatedNotices && event->pos().x() < statusX) {
            ...
            if (state != 0) {
                ...
                const QStringList d    = index.data(ModRole::TranslationDetail).toStringList();
                const QStringList head = d.value(0).split('\t');
                const int blank        = d.indexOf(QString(), 1);
                const QStringList plugins = (blank > 1) ? d.mid(1, blank - 1) : QStringList();
                const QStringList samples = (blank >= 0) ? d.mid(blank + 1) : QStringList();

                QString tip;
                if (state == 1) {
                    tip = tr("Nothing in this list translates this mod.\n"
                             "It carries %1 line(s) of text the player sees.")
                              .arg(head.value(1));
                } else {
                    tip = tr("%1 leaves %2 of %3 shared line(s) unchanged.")
                              .arg(head.value(0), head.value(2), head.value(3));
                }
                if (!plugins.isEmpty()) {
                    tip += "\n\n" + tr("Plugin(s):");
                    for (const QString &p : plugins) tip += "\n  " + p;
                }
                if (!samples.isEmpty()) {
                    tip += "\n\n" + tr("Still the same in both:");
                    for (const QString &s : samples)
                        tip += "\n  " + s.left(60);
                }
                // Said plainly, because the count alone invites the wrong
                // conclusion: names that are the same word in both languages
                // are supposed to show up here.
                tip += "\n\n" + tr("Proper nouns are counted too, so a small "
