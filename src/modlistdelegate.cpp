                            ...
                            const QModelIndex &index) const
{
    ...
    // Conflict warning icon: orange triangle with ! (slot 1).
    // Only on the selected row - an orange triangle on every mod that shares
    // a file with another turns the list into a wall of warnings. Click-to-
    // reveal, like MO2's on-demand overwrite display.
    if (index.data(ModRole::HasConflict).toBool()
     && (option.state & QStyle::State_Selected)) {
        ...
        painter->setRenderHint(QPainter::Antialiasing, true);
        painter->setPen(Qt::NoPen);
        painter->setBrush(QColor(210, 100, 0));
