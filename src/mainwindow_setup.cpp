...
void MainWindow::setupMenuBar()
{
    ...
    // The dropdown pins the pinned() games - OpenMW, the Bethesda deploy
    // targets (Oblivion, FNV, Fallout 4, Starfield) and the Enderal pair (each
    // adapter owns its own pinned() flag, so that list lives there, not here).
    // OpenMW and FNV are the exercised ones; the Bethesda deploy path is
    // classified but the real-install pass has not been run, so treat it as
    // experimental. This toggle surfaces the rest of the legacy game list.
    // Off by default.
    {
...
void MainWindow::setupToolbar()
{
    ...
    // -- Conflict notices toggle ---
    // Next to the theme button, same shape: hides the overwrite arrows,
    // captions and partner highlight when the list is already understood and
    // the noise is not wanted. Label names the state the click will switch TO.
    m_conflictNoticesBtn = new QToolButton(tb);
    m_conflictNoticesBtn->setToolButtonStyle(Qt::ToolButtonTextOnly);
    connect(m_conflictNoticesBtn, &QToolButton::clicked,
            this, &MainWindow::onToggleConflictNotices);
    tb->addWidget(m_conflictNoticesBtn);
    updateConflictNoticesButton();

    // -- Untranslated-mods toggle ---
    // Third of the row, same shape again. Unlike its neighbour this one drives
    // a scan rather than just hiding what a scan already produced, so the first
    // click takes a moment on a big list.
    m_untranslatedBtn = new QToolButton(tb);
    m_untranslatedBtn->setToolButtonStyle(Qt::ToolButtonTextOnly);
    connect(m_untranslatedBtn, &QToolButton::clicked,
            this, &MainWindow::onToggleUntranslatedNotices);
    tb->addWidget(m_untranslatedBtn);
...
void MainWindow::restyleToolbarTextButtons()
{
    ...
    // margin, on top of padding: QToolBar adds no gap of its own between items
    // (measured 0px under Breeze, 1px under Fusion), so the breathing room
    // between the borderless buttons - Good States, Forbidden Mods, Restore
    // Backup - is really their own 8px horizontal padding reading as
    // whitespace. These two are bordered, so the same non-gap reads as two
    // boxes welded together. A stylesheet margin is painted INSIDE the widget
    // rect, so 4px a side insets the borders and puts a matching gap back
    // without moving anything else on the toolbar.
    const QString toggleStyle = QStringLiteral(
        ...
        "  color: %2;"
        "}"
        "QToolButton:hover { background: rgba(127,127,127,0.18); }").arg(mid, txt);
    if (m_themeBtn)            m_themeBtn->setStyleSheet(toggleStyle);
    if (m_conflictNoticesBtn)  m_conflictNoticesBtn->setStyleSheet(toggleStyle);
    if (m_untranslatedBtn)     m_untranslatedBtn->setStyleSheet(toggleStyle);
}
...
void MainWindow::setupCentralWidget()
{
    ...
    // Parented to the viewport, not the window: it centres on the list the
    // scan is about, and stays put while the list scrolls underneath.
    m_translationOverlay = new ScanOverlay(m_modList->viewport());
    m_deployOverlay      = new ScanOverlay(m_modList->viewport());
    ...
    // Two different sticky banners now, so the click has to know which one is
...
void MainWindow::switchToGame(int idx)
{
    ...
    // Kill the banner first thing: it describes the game being left. "All mods
    // are up to date." lingering over another game's list is a claim about a
    // list that is no longer on screen, and it may well be false for the new
    // one. The view sort goes with it - the list below is rebuilt from disk in
    // saved order, so nothing is sorted any more (persistence keys off the
    // per-row stamps, not this flag, so clearing it cannot affect the save).
