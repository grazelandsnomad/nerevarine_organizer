...
// TES3 BSA layout, all little-endian. We only want filenames, so everything
// past the name block is skipped.
//   [ 0 ..  4)  version = 0x00000100
//   [ 4 ..  8)  hashOffset, relative to end of the 12-byte header (hash table
//               starts at absolute 12+hashOffset)
//   [ 8 .. 12)  fileCount (N)
//   [12 .. 12+8N)   file size/offset records (unused)
//   [12+8N .. 12+12N)   name offsets (uint32 into the name block)
//   [12+12N .. 12+hashOffset)   packed null-terminated filenames
//   [12+hashOffset .. +8N)   8-byte hashes (ignored)
//   [ file data ... ]
...
QStringList listTes3BsaFiles(const QString &bsaPath)
{
    ...
    if (fileCount > 1'000'000) return {};      // reject absurd counts

    // Name block runs from end of the name-offset array to the hash table.
    ...
    // Skip the 8N-byte file-record array, read name offsets.
    ...
    for (quint32 i = 0; i < fileCount; ++i) {
        ...
        // Null-terminated; find the terminator without running past the block.
        const char *p = base + off;
        const char *e = p;
        while (e < maxEnd && *e) ++e;
        if (e == maxEnd) return {};           // unterminated -> malformed

        // Forward-slash + lowercase to match conflict-map keys.
