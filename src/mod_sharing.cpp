...
ModEntry makeSharedRow(const ModEntry &source, bool copyConfig)
{
    ...
    // Shared install identity - kept verbatim so both profiles point at the
    // same folder / Nexus page.
    ...
    // installToken / prevModPath / mergeTargetPath / intendedModPath and the
    // transient conflict / dependency / missing-master fields stay at their
    // ModEntry defaults: a shared row is a fresh, independent row.
    return e;
}
