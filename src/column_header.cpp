    ...
    : QObject(parent)
{
    ...
    // Date and size buttons get inserted later by setDate/SizeSortButton().
    // Final order: [name, status, date, relTime, annot, size, video].
    ...
    // index 5 (after date insert): size button
...
void ColumnHeader::setDateSortButton(QPushButton *btn)
{
    ...
    // Between status (1) and relTime (2).
...
void ColumnHeader::setSizeSortButton(QPushButton *btn)
{
    ...
    btn->setMouseTracking(true);
    btn->installEventFilter(this);
    // Between annot (4, once date is in) and video (5).
...
void ColumnHeader::onWindowStateChanged(bool maximized)
{
    ...
    loadWidthsForCurrentState();
    apply();
    // changeEvent fires before Qt re-lays out the viewport for the
    // maximise/restore, so viewport()->geometry() is stale here. Realign once
    // after the pending layout pass.
    updateScrollMargin();
...
int ColumnHeader::separatorAt(int x) const
{
    ...
    // Bar order: name | status | date | relTime | annot | size | video.
    // Walk leftward from the right edge, peeling each column to the right before
    // hit-testing its separator. Video sits right of size so it must be peeled
    // first; skipping it offsets every separator by wVideoReview px.
...
{
    // Scrollbar show/hide changes viewport width; re-fix the bar's right margin
