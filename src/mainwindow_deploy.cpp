...
static QList<bethesda_deploy::DeploySource> gatherBethesdaSources(QListWidget *list, const GameAdapter *adapter);
...
void MainWindow::onDeployBethesda()
{
    ...
    const auto sources = gatherBethesdaSources(m_modList, adapter);
    if (sources.isEmpty()) {
...
void MainWindow::onInspectDeployment()
{
    ...
    f.dataRootCount = int(gatherBethesdaSources(m_modList, adapter).size());
