...
// A BAIN package folder starts with a number, optionally a letter, then a
// separator + label: "00 Core", "01 Optional", "10a Alt Meshes", "02_Patch".
// Anchored so "100 Mods Merged" style names still qualify but a plain "Core"
// (no number) does not.
...
// Folder names that are themselves an OpenMW data root, not a BAIN package.
// If a top-level subfolder is one of these the archive is plain mod data, not
// a numbered package set. Mirrors install_layout's kDataRootNames.
...
bool looksLikeBain(const QString &modPath)
{
    ...
    // Require EVERY top-level folder to be numbered: a mix (one numbered folder
    // beside ordinary ones) is some other layout, not a BAIN package set.
...
QString stage(const QString &modPath, const QStringList &chosenNames)
{
    ...
    // packages() order is numeric, so merging in that order makes a
    // higher-numbered package overwrite a lower one (last writer wins, handled
    // inside fomod_copy::copyContents).
    for (const Package &p : packages(modPath)) {
        if (chosen.contains(p.name))
            fomod_copy::copyContents(p.path, stageDir);
    }

    // Empty result (every chosen package was empty) -> signal "nothing staged".
    QDir sd(stageDir);
