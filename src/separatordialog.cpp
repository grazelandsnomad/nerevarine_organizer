...
static const QList<Preset> PRESETS = {
    ...
    // Utility: gold-on-dark-slate, unused by other presets so framework
    // sections don't clash with content categories.
    {"🧩", "sep_preset_utility",      QColor("#2C2F3A"), QColor("#FFD700")},
    // NPCs: burnt-orange/deep-teal, kept distinct from Characters (pink) and
    // Companions (mauve).
    {"🧑", "sep_preset_npcs",         QColor("#F4A261"), QColor("#264653")},
    // Flora: dark green on pale lime, inverted brightness vs the other greens
    // (Immersion/Landscape/Animations/Towns) so plants stay distinct.
...
{
    // Right-click "Remove suggested separator" hides presets. Store hidden keys
    // rather than rewriting PRESETS, so clearing the setting re-enables them.
...
void SeparatorDialog::onSearchChanged(const QString &text)
{
    ...
    // Exactly one match: auto-select and apply it.
